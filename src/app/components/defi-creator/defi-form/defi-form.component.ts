...
import { DefiType, Defi } from 'src/app/models/defi';
...
export class DefiFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.defiInput) {
      ...
      this.questionService
        .getQuestionByidDefi(this.defiInput.idDefi)
  ...
  OnSubmit(): void {
    ...
    const selectedArret = this.arretService.featureToArret(
      this.defiForm.get('arret')?.value
    );
    ...
    this.arretService.getArretByCode(selectedArret.code as string).subscribe(
      ...
      (error: HttpErrorResponse) => {
        if (error.status === 404) {
          this.arretService
            .createArret(selectedArret)
            .subscribe((arret) => this.createOrUpdateDefi(arret));
        }
      }
    );
  }
  ...
  createOrUpdateDefi(arret: Arret): void {
    ...
    if (this.defiInput) {
      ...
      this.defiService
        .update(this.leDefi.idDefi, this.leDefi as Defi)
        .subscribe((defi) => {
          console.log(defi);
          ...
          for (const question of this.questions) {
            if (question.idDefi) {
              this.questionService
                .update(question.idDefi, question as Question)
                .subscribe();
            } else {
  ...
  editQuestion(editedQuestion: Partial<Question>): void {
    ...
    const questionFound = this.questions.find(
      (question) => question.numero === editedFullQuestion.numero
    );
    if (questionFound) {
