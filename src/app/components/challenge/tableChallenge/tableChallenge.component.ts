...
import { Chami } from 'src/app/models/chami';
...
export class TableChallengeComponent implements AfterViewInit {
  ...
  ngOnInit() {
    const DATA_SOURCE_PLUS: DefiPlus[] = this.DATA_SOURCE.map((defi: Defi) => ({
      ...defi,
  ...
  async ouvrirPageProfil(idProfil: string): Promise<void> {
    await this.router.navigate([`/profil/${idProfil}`], {
      relativeTo: this.route,
    });
  }

  getChamisByUid(uid: string): string {
    const pseudo: string | undefined = this.chamiData.find(
      (chami: Chami) => chami.uid === uid
    )?.pseudo;
    if (pseudo) {
      return pseudo;
    } else {
  ...
  getArretByIdA(idA: number): string {
    const arret: string | undefined = this.arretData.find(
